...
exports.AdminEditContainer = ({ currentInputEl, index, breadth, depth, placeholder, temp, handleChange, handleSubmit, parentBreadth, parentDepth }) => {
  ...
  const handleDepthPlaceholder = (depth) => {
    ...
    depth === 0 ? 'New Title'
      : depth === 1 ? 'New Section'
        : depth === 3 ? 'New Minor Detail'
          : depth === 4 ? 'Highlights Title'
            : depth === 5 ? 'Highlights Detail'
              : null;
  }
  ...
  return (
    <div className={
      ...
      <form
        className='inputForm'
        data-depth={depth}
        data-parentbreadth={parentBreadth}
        data-parentdepth={parentDepth}
      ...
      >
        <input autoFocus
          ...
          data-temp={temp}
          data-breadth={breadth}
          placeholder={handleDepthPlaceholder(depth)}
          value={temp}
          type='text'
...
exports.AdminDisplayContainer = ({ containerRef, index, depth, handleClick, displayItem, breadth, parentBreadth }) => {
  ...
  const handleClassName = (depth) => {
    return depth === 0 ? `${'flexRowTitle'} ${'flexRowSmallTitle'}` :
    depth === 1 ? `${'flexRowTitle'} ${'depthOne'}` : `flexRowTitle`
  }
